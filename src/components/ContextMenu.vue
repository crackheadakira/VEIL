<template>
    <div v-if="showDropdown" ref="contextMenu" @mouseleave="handleMouseLeave()" @mouseenter="reEntered = true"
        ...
        class="absolute z-50 flex h-fit w-fit cursor-pointer select-none flex-col rounded-md border border-stroke-100 bg-card p-2 text-supporting">
        <small @click="$emit('add-to-queue', userCoords), showDropdown = false"
            class="rounded-md p-2 duration-150 hover:bg-stroke-100 hover:text-text">Add to Queue</small>
