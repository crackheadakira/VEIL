<template>
    <div
        class="relative flex h-fit w-56 cursor-pointer select-none flex-col gap-2 rounded-md border border-stroke-100 bg-background text-supporting">
        ...
        <Transition enter-from-class="-translate-y-[25%] opacity-0" leave-to-class="-translate-y-[25%] opacity-0"
            ...
            <div v-if="showOptions"
                ...
                <small v-for="option in options" @click="handleSelect(option), $emit('dropdownSelected', option)"
                    class="p-3 duration-150 hover:bg-card hover:text-text">
                    {{ option }}
                </small>
            </div>
        </Transition>
